...
class JobPagamentosIT {
    ...
    @Test
    fun partialFailureDoesNotRollbackBatch() {
        // 8 valid + 4 invalid, all claimed in a single batch (default batch-size 100)
        // and therefore committed within one TransactionTemplate transaction.
        // A failure on the invalid rows must not roll back the committed valid rows.
        repeat(8) { i -> requestRepository.save(validRequest(i)) }
        repeat(4) { i -> requestRepository.save(invalidRequest(200 + i)) }

        service.runLoop()

        assertThat(requestRepository.countByStatus(RequestStatus.PROCESSED)).isEqualTo(8)
        assertThat(requestRepository.countByStatus(RequestStatus.FAILED)).isEqualTo(4)
        assertThat(requestRepository.countByStatus(RequestStatus.PENDING)).isEqualTo(0)
        // Valid rows persisted their Pagamento despite the failures in the same batch.
        assertThat(pagamentoRepository.count()).isEqualTo(8)
        // A single Lote was resolved for the batch.
